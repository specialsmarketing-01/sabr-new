...
const iconClass = 'h-8 w-8 shrink-0 opacity-90';

const svgProps = { viewBox: '0 0 24 24' as const, fill: 'none' as const, stroke: 'currentColor', strokeWidth: 1.8, strokeLinecap: 'round' as const, strokeLinejoin: 'round' as const, 'aria-hidden': true, style: { width: 32, height: 32, display: 'block' } };

function CarIcon({ className }: { className?: string }) {
  return (
    <svg className={className ?? iconClass} {...svgProps}>
...
export function ServiceIcon({
  ...
  if (!Icon) {
    return <span className={className ?? iconClass} style={{ width: 32, height: 32, display: 'block' }} aria-hidden />;
  }
  return (
    <span className="inline-flex items-center justify-center text-white [&_svg]:text-white" style={{ color: 'white' }} aria-hidden>
